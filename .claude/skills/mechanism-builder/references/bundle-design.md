...
- **Rule との関係が不明確**: `.claude/templates/cross-review/` のような独自フォルダは、対応する Rule との参照関係が分かりにくい
...
### 8.1 writing-guideline（執筆ガイドライン Skill）

構造:
```text
.claude/skills/writing-guideline/
├── SKILL.md
└── references/
    ├── common/
    │   ├── 01_構造設計編.md
...
設計判断:
- 補助ファイル多数（共通 3 + 種別別 5）→ Skill 化
- `user-invocable: false`: 編集時の path-scoped 自動ロード専用
- references/ は「構造設計編」「執筆中編」等のフェーズ別に分割
- `templates/` `examples/` `scripts/` は不要（ガイドラインのため）

### 8.2 cross-review（クロスレビュー Skill）
...
└── templates/
    ├── 論理整合性観点_template.md
    ├── 実用性観点_template.md
    └── 作業指示者+人間読み手観点_template.md
```

設計判断:
- 補助ファイル（テンプレ 3 種）+ 詳細運用仕様（runtime-rules.md）→ Skill 化
- 旧構造: Rule（`.claude/rules/cross-review-runtime.md`）+ 独自テンプレフォルダ（`.claude/templates/cross-review/`）→ Skill 統合
- 移行時は旧パスをリダイレクト残置（過去レビュー報告書からのリンク維持）
- `user-invocable: false` + `paths: ["**/レビュー/**/*.md", ".claude/skills/cross-review/**/*.md"]`
