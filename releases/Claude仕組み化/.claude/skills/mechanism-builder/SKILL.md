...
提案ファイルの出力先は **受け取り側プロジェクトの CLAUDE.md に明記された「一時ファイル・中間成果物の出力先ルール」** に従う。スニペット例は同梱の `mechanism-builder-snippet.md` 参照（CLAUDE.md に取り込んで利用）。

該当階層の CLAUDE.md にルールがない場合は、推測せず作業指示者に確認する。スクリプトのデフォルト出力先は `.claude/workspace/mechanism-builder/<対象名>/proposal-YYYY-MM-DD.md`。
...
受け取り側プロジェクト:
- CLAUDE.md「一時ファイル・中間成果物の出力先」ルール（同梱の `mechanism-builder-snippet.md` 参照）
