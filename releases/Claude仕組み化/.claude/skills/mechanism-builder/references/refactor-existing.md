...
例:
- `/mechanism-builder my-skill` — Skill / Rule のいずれかが対象（自動解決）
- `/mechanism-builder skill:my-skill` — Skill に限定
- `/mechanism-builder rule:my-rule` — Rule に限定
...
- 出力先（一時ファイル）: 受け取り側プロジェクトの一時ファイル出力先ルールに従う。スクリプトのデフォルトは `.claude/workspace/mechanism-builder/<対象名>/.analysis.md`

## 4. 出力先ルール

提案ファイルの出力先は **受け取り側プロジェクトの CLAUDE.md に明記された「一時ファイル・中間成果物の出力先ルール」** に従う。スニペット例は同梱の `mechanism-builder-snippet.md`（受け取り側 CLAUDE.md に取り込んで利用）。

### 解決フロー

1. 該当階層の CLAUDE.md に出力先ルールがあればそれに従う
2. なければ作業指示者に確認
...
- 出力先ルール: 受け取り側プロジェクトの CLAUDE.md（スニペット例は同梱の `mechanism-builder-snippet.md`）
