...
- **⚠ Phase 6 で v1.0.0 を切った後に再 publish すると「変更なし」になりうる**（payload が RC1 と同一のため）。publish-share は commit 失敗を検知して中止する設計なので、**空コミットになるケースで `✗` を返す可能性がある**。5b で RC1 を publish 済みなら、v1.0.0 は**タグを打つだけで再 publish しない**（配布物は同一）か、`--allow-empty` 相当の扱いを決めてから実行する。**Phase 6 着手時に実測して確定すること**。
