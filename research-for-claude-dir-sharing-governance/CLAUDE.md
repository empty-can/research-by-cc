...
- [ ] Phase 5a（C-BCP へ起動装置・ルート `.gitattributes`・雛型 `CLAUDE.md.sample` を移管。C-BDK 側は `CLAUDE.md.example` を削除）→ Phase 5b（`/security-review` → `publish-share --ref v1.0.0-RC1` → C-BCP submodule bump → **受入検証**）→ Phase 6（受入 pass なら `v1.0.0` 昇格・fail なら RC2）
