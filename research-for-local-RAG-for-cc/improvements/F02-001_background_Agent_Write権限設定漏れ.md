...
**追加観察と運用案（2026-05-07）**: チーム共有 `.claude/settings.json` および個人ローカル `.claude/settings.local.json` の両方を確認したが、Opus Agent 実行中の 3 件 prompt は **いずれにも永続記録されていない**。これは 2 択（許可/拒否）モードで「このセッション上常に許可」が選択不可だった場合の挙動と整合する（作業指示者観察）。**運用案（作業指示者ジャストアイデア）**: 2 択モードで 1 度敢えて拒否すれば公式仕様の `/permissions` Recently denied タブに記録されるため、事後にテキスト取得可能になる。Agent 作業の手戻り対策として「拒否されても 1 度なら（拒否されたコマンドが必要不可欠であれば）同じコマンドで再依頼してよい」を Agent 委任プロンプトのルールに追加する案が示された。**メインセッション見解**: 採用価値あり、ただし常時適用は手戻りコストが過剰（Agent 起動毎に拒否 → 再依頼の手戻りでタスクのリードタイムが 1.5〜2 倍）。**「観察モードフラグ運用」**（特定の実証データ収集タスク時のみ拒否ルール適用、通常運用では「常に許可」で手戻り最小化）として `agent-permission-runtime.md` §2 検出ベースワークフローへの組み込み候補。連続 prompt 時の観察データ偏り（最初の 1 件のみ詳細記録、後続は通常承認）と、Remote Control + スマホアプリ運用下での `/permissions` タブ参照可否（テキストコピー仕様の影響）も併せて検討事項。クロージング時に F02-013 改善案とも統合判断対象
...
- 2026-05-07: §6 特記事項末尾に「追加観察と運用案」段落を新設。チーム共有 settings.json および個人ローカル settings.local.json のいずれにも 3 件 prompt が永続記録されていなかった事実を確認（2 択モード「常に許可」非対応の挙動と整合）。作業指示者ジャストアイデア（2 択モード時に 1 度敢えて拒否することで `/permissions` Recently denied タブに記録残し、Agent 作業の手戻り対策として「1 度なら必要不可欠なコマンドの再依頼を許可」をルール化する運用案）と、メインセッション見解（採用価値ありだが常時適用は手戻りコスト過剰、観察モードフラグ運用として `agent-permission-runtime.md` §2 への組み込み候補）を併記。クロージング時に F02-013 改善案とも統合判断対象として記録
